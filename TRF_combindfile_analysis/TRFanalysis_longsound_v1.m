...
numTrial = length(ind_Gd_Spat); %number of good front trials
...
stim_comb = MskSound; %combined stimulus (initialization)

for i = 1:numTrial-1
    stim_comb = CrossFader(stim_comb, MskSound, OvlpDur*fsSound); %combine stimuli
end

%EEG
epochs_Pos = resample(double(epochs_Gd(1:streamdur,1:numChMsk,OstSpt(GdTr_final)==TgSpt(spat_ind))),fsSound,fsEEG); %epochs of front/back and resample to adjust the length
epochs_Pos = epochs_Pos(1:size(MskSound,1),:,:); %adjust the length (the gap between EEG and sound (7.9167e-04 sec) is small as possible to be ignored)
epochs_Pos_referred = epochs_Pos(:,[Hotch(1) Hotch(2)],:) - (epochs_Pos(:,[Coldch(1) Coldch(2)],:) + epochs_Pos(:,[Coldch(2) Coldch(1)],:))/2; %refer O1 and O2
EEG_comb_Fz = epochs_Pos_referred(:,1,1); %combined EEG signals (initialization) for Fz
EEG_comb_Cz = epochs_Pos_referred(:,2,1); %combined EEG signals (initialization) for Cz

for i = 1:numTrial-1
    EEG_comb_Fz = CrossFader(EEG_comb_Fz, epochs_Pos_referred(:,1,i+1), OvlpDur*fsSound); %combine EEG signals
    EEG_comb_Cz = CrossFader(EEG_comb_Cz, epochs_Pos_referred(:,2,i+1), OvlpDur*fsSound);
end
...
%% Plot figure 
chan=1; %temporarily using mono channel EEG

%%% Fz %%%
% Plot STRF
figure
subplot(2,2,1), mTRFplot(model_Fz,'mtrf','all',chan,[-50,350]);
title(sprintf('Speech STRF (%s)', ch1)), ylabel('Frequency band'), xlabel('')
...
title(sprintf('Speech TRF (%s)', ch1)), ylabel('Amplitude (a.u.)')

% Plot GFP
subplot(2,2,4), mTRFplot(model_Fz,'gfp','all','all',[-50,350]);
title('Global Field Power')

figtitle = sprintf('TRF %s Spat:%s Ch:%sf', experiment_name, Spat(spat_ind), ch1);
sgtitle(figtitle)
pdfname = sprintf('%s/TRF_%s_Sp%s_Ch%s.pdf', outfolder, experiment_name, Spat(spat_ind), ch1);
saveas(gcf, pdfname)
...
title(sprintf('Speech STRF (%s)', ch2)), ylabel('Frequency band'), xlabel('')
...
title(sprintf('Speech TRF (%s)', ch2)), ylabel('Amplitude (a.u.)')

% Plot GFP
subplot(2,2,4), mTRFplot(model_Cz,'gfp','all','all',[-50,350]);
title('Global Field Power')

figtitle = sprintf('TRF %s Spat:%s Ch:%sf', experiment_name, Spat(spat_ind), ch2);
sgtitle(figtitle)
pdfname = sprintf('%s/TRF_%s_Sp%s_Ch%s.pdf', outfolder, experiment_name, Spat(spat_ind), ch2);
saveas(gcf, pdfname)
