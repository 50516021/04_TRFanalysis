%%% mTRF EEG analysis for EDF/CSV - step 1 %%% 
%%% - epoching 
...
load(metadata_file); 
save([outfolder extractAfter(metadata_file, filepath)],'metadata'); %copy metadata file 

numTrial = size(metadata,1); %amount of trials [manual]
...
numTrial = length(stimOnsets); %amount of trials
...
epochs = zeros(fix(streamdur_epoch/fsOgnl*fs),numCh,numTrial); %epochs(EEG signal, channel, trial)
