...
prompt = 'Chose re-referencing channel:';  % prompt message
...
%%% chanel info
if ICAopt==1 && proInd==1 %use subtracted (ICAed) epoch
    epochs = saveEp; 
elseif proInd == 2 %use all channelss epoch
    ...
    epochs = epochs(:,ch_ind_ref,:)-(epochs(:,Coldch(1),:)+epochs(:,Coldch(2),:))/2; %subtraction from center channel
end
...
for i = 1:length(inst_flg)
    for j =1:size(stim,3)
        ...
        disp(strcat(filename, ' model has been saved'))
